...
def lambda_handler(event, context):
    """
    AWS Lambda function handler for applying a blur effect to images stored in S3.

    This function is triggered by an event containing information about an image in S3.
    It downloads the image, applies a blur filter, and uploads the transformed image
    back to S3 in a 'blurred/' prefix.
    ...
    Returns:
        dict: A dictionary containing:
            ...
            - SourceBucket (str): The name of the S3 bucket where the blurred image is stored.
            - ObjectKey (str): The key of the blurred image in S3.
