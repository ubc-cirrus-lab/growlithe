...
def growlithe_extract_param_taint(taint_label, event):
    ...
    if 'detail' in event.keys() and 'bucket' in event['detail']:
        default_id = event['detail']['bucket']['name'] + event['detail']['object']['key'] + event['time']
    ...
    if 'Records' in event.keys():
        GROWLITHE_TAINTS[taint_label].add(f"DYNAMODB_TABLE:{event['Records'][0]['dynamodb']['Keys'][list(event['Records'][0]['dynamodb']['Keys'].keys())[0]]['S']}")
    # S3 trigger
    if 'detail' in event.keys() and 'bucket' in event['detail']:
        GROWLITHE_TAINTS[taint_label].add(f"S3_BUCKET:{event['detail']['bucket']['name']}:{event['detail']['object']['key']}")
...
def growlihte_add_s3_object_taint(taint_label, bucket, object_key):
    GROWLITHE_TAINTS[taint_label].update(set(bucket.Object(object_key).metadata.get('growlithe_taints', "").split(",")))
...
def growlithe_save_s3_taint(taint_label, bucket, object_key):
    ...
    object.metadata.update({'growlithe_taints': ','.join(GROWLITHE_TAINTS[taint_label])})
    object.copy_from(CopySource={'Bucket': bucket.name, 'Key': object_key}, Metadata=object.metadata, MetadataDirective='REPLACE')