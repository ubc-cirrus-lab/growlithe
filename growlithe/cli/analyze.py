...
from growlithe.common.tasks_config import CREATE_CODEQL_DB, GENERATE_EDGE_POLICY, RUN_CODEQL_QUERIES
...
def analyze(config):
    ...
    click.echo("Analyzing the application...")
    ...
    if config.app_config_type == 'SAM':
        ...
        click.echo(f"Unsupported app_config_type: {config.app_config_type}", color='red')
        return
