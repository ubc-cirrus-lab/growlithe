...
module LambdaInvoke {
  class LambdaInvoke extends DataFlow::CallCfgNode {
    ...
    DataFlow::Node getFunctionName() { result in [this.getArg(0), this.getArgByName("FunctionName")] }
    DataFlow::Node getPayload() { result in [this.getArg(2), this.getArgByName("Payload")] }

    string getFunctionNameAsResource() { result = "LAMBDA_INVOKE:" + Utils::strRepr(getFunctionName()) }
